  ...
  <q-page class="page-wrapper">
    <q-card class="page-card">
      <q-card-section>
        ...
        <p/>
        <p>작성자: {{ successCase.username }}</p>
        <p>등록일: {{ formatDate(successCase.createdDate) }}</p>
      </q-card-section>

      <q-card-section>
        <div class="content" v-html="successCase.content"></div>
        <q-btn v-if="authStore.username === successCase.username" label="삭제" color="negative"
               @click="deleteSuccessCase" icon="delete" flat dense size="sm"/>
      </q-card-section>
      <q-card-section>
        <div class="comments-section">
          <q-input
            v-model="newComment"
            :placeholder="authStore.isLoggedIn?'댓글을 입력하세요.' :'로그인 후에 이용할 수 있습니다.'"
        ...
        <div class="comments-list">
          <q-card v-for="comment in comments" :key="comment.commentId" class="comment-card" flat bordered>
            <q-card-section>
              <div class="comment-header">
                ...
                <div>
                  <q-btn icon="edit" flat dense v-if="authStore.username === comment.username" label="수정"
                         @click="toggleEditComment(comment.commentId)" size="sm"/>
                  <q-btn v-if="authStore.username === comment.username" label="삭제" color="negative"
                         @click="deleteComment(comment.commentId)" icon="delete" flat dense size="sm"/>
                </div>
              </div>
              <div class="comment-content">
                <q-input v-if="editCommentId === comment.commentId"
              ...
              <q-btn v-if="editCommentId === comment.commentId" label="저장" color="primary"
                     @click="saveEditComment(comment.commentId)" flat dense/>
              <q-btn flat dense color="primary" label="답글 보기" @click="toggleReplies(comment.commentId)"/>
              <div v-if="repliesVisible[comment.commentId]" class="replies">
                <q-card v-for="reply in comment.replies" :key="reply.replyId" class="reply-card" flat bordered>
                  <q-card-section>
                    <div class="reply-header">
                      <div>
                        <div class="author">{{ reply.username }}</div>
                        <div class="date">{{ formatDate(reply.createdDate) }}</div>
                      </div>
                      <div>
                        <q-btn v-if="authStore.username === reply.username" label="수정" size="sm"
                               @click="toggleEditReply(comment.commentId, reply.replyId)" icon="edit" flat dense/>
                        <q-btn v-if="authStore.username === reply.username" label="삭제" color="negative" size="sm"
                               @click="deleteReply(comment.commentId, reply.replyId)" icon="delete" flat dense/>
                      </div>
                    </div>
                    <div class="reply-content">
                      <q-input v-if="editReplyId === reply.replyId"
                    ...
                    <q-btn v-if="editReplyId === reply.replyId" label="저장" color="primary"
                           @click="saveEditReply(comment.commentId, reply.replyId)" flat dense size="sm"/>
                  </q-card-section>
                </q-card>
                <q-input
                  v-model="newReply[comment.commentId]"
                  :placeholder="authStore.isLoggedIn?'답글을 입력하세요.' :'로그인 후에 이용할 수 있습니다.'"
                  :disable="!authStore.isLoggedIn"
                  outlined
                  dense
                  class="reply-input"
...
const fetchComments = async () => {
  try {
    ...
    comments.value = response.data.sort((a, b) => new Date(b.createdDate) - new Date(a.createdDate));
...
const fetchReplies = async (commentId) => {
  try {
    ...
    if (comment) {
      comment.replies = response.data.sort((a, b) => new Date(b.createdDate) - new Date(a.createdDate));
    }
  } catch (error) {
...
const postReply = async (commentId) => {
  ...
  try {
    await customApi.post(`/comment/${commentId}/reply`, {content: newReply.value[commentId]});
...
const saveEditComment = async (commentId) => {
  try {
    await customApi.patch(`/success-case/${caseId}/comment/${commentId}`, {content: newCommentContent.value});
    const comment = comments.value.find(c => c.commentId === commentId);
    comment.content = newCommentContent.value;
    editCommentId.value = null;
    newCommentContent.value = '';
    await fetchComments(); // 업데이트된 댓글 목록 가져오기
  } catch (error) {
...
const saveEditReply = async (commentId, replyId) => {
  try {
    await customApi.patch(`/comment/${commentId}/reply/${replyId}`, {content: newReplyContent.value});
    const comment = comments.value.find(c => c.commentId === commentId);
    const reply = comment.replies.find(r => r.replyId === replyId);
    reply.content = newReplyContent.value;
    editReplyId.value = null;
    newReplyContent.value = '';
    await fetchReplies(commentId); // 업데이트된 답글 목록 가져오기
  } catch (error) {
